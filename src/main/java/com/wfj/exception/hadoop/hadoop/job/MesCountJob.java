...
/**
 * 统计未处理的有效的异常数据
 * @ClassName: MesCountJob
 * @Description: TODO(这里用一句话描述这个类的作用)
 * @author ZJHao
 * @date 2015-8-28 下午4:26:54
 *
 */
public class MesCountJob {
...
//		HTable htb=new HTable(conf, tableName);
		...
		job.setJarByClass(MesCountJob.class);
		
//		Scan scan=new ScanBase().findSysMess("01");
		Scan scan=new ScanUtil().findSysMessScan(sysCode);
		
		TableMapReduceUtil.initTableMapperJob(tableName, scan, MesCountMapper.class, Text.class, Text.class, job);
		TableMapReduceUtil.initTableReducerJob(tableName,MesCountReduce.class , job);
...
//		System.out.println(JSONObject.toJSONString(sysReq));
	...
	public static void main(String[] args) throws Exception {
		...
		final MesCountJob mesCountJob=(MesCountJob) SpringHandlerBean.getContext("mesCountJob");
		mesCountJob.runJob("02");
		for(int i=0;i<TableParamCommon.sysCodes.length;i++){
			final String k=TableParamCommon.sysCodes[i];
